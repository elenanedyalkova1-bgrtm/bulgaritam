...
const harmlessSlugRows=parseCsv(`brand_name,brand_slug,name_bg,slug,product_url\nÁtelier & Co,atelier-co,One,one,https://example.com/one\nAtelier Co,atelier-company,Two,two,https://example.com/two\n`);
const harmlessSlugPlan=await prepareImport(harmlessSlugRows,{products:[],brands:[]});
assert.equal(harmlessSlugPlan.brandsToCreate.length,1,"case/accent/punctuation and harmless slug differences share one planned Brand");
assert.equal(harmlessSlugPlan.invalid,0);
const conflictingRows=parseCsv(`brand_name,brand_slug,name_bg,slug,product_url\nFirst Brand,shared-slug,One,conflict-one,https://example.com/one\nSecond Brand,shared-slug,Two,conflict-two,https://example.com/two\n`);
const conflictingPlan=await prepareImport(conflictingRows,{products:[],brands:[]});
assert.equal(conflictingPlan.invalid,1,"genuinely conflicting CSV identities require review");
