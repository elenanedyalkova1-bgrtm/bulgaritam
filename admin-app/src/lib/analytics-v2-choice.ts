...
export function choiceOrientation(summary:{visitors:number;actions:number},taxonomy:ChoiceRow[]){const base=`${summary.visitors} ${visitorsWord(summary.visitors)} ${summary.visitors===1?"е направил":"са направили"} ${summary.actions} ${choicesWord(summary.actions)}.`;const top=taxonomy[0],tied=top&&taxonomy[1]?.actions===top.actions;if(!top||tied)return`${base} Данните все още са твърде малко, за да ги приемаме като обща тенденция.`;const verb=top.dimension==="product_type"?"избран":"избрана",group=top.dimension==="category"?"категориите":top.dimension==="subcategory"?"подкатегориите":"типовете продукти";return`${base} Сред ${group} „${choiceValueLabel(top.dimension,top.label)}“ е ${verb} ${top.actions} пъти от ${top.visitors} ${visitorsWord(top.visitors)}, но данните все още са твърде малко, за да приемаме това като обща тенденция.`}
